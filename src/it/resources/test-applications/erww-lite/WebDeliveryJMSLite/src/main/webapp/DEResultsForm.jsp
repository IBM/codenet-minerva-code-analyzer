...
<jsp:setProperty name="dEBean" property="warehouseId"
	value='<%=Short.valueOf(request.getParameter("warehouseId")).shortValue()%>' />
<jsp:setProperty name="dEBean" property="carrierId"
	value='<%=Short.valueOf(request.getParameter("carrierId")).shortValue()%>' />
...
//Execute client bean method for manual input
try {
   dEBean.webManualInput();
   
   // We have succsess, unless we got an exception
   status = "SUCCESSFUL";
   status2 = "none";
}
catch (Throwable t) {
    // We failed, report exception in status2
	tt=t;
	status = "FAILED";
	if (t.getCause()!=null) {
