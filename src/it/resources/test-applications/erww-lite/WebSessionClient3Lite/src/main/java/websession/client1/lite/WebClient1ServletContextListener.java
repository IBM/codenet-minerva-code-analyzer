 ...
 */
@WebListener
public class WebClient1ServletContextListener implements ServletContextListener, ServletContainerInitializer {
	...
	String cookieType = System.getProperty("SESSION_COOKIE_TYPE");
	if (cookieType.contains("STARTUP")){
		try {
		...
		try	{         
			...
			irwwBase.debugOut("<<< " + classname + "(System.getProperty(SESSION_TRACKING_MODE).startsWith(URL) = " + (System.getProperty("SESSION_TRACKING_MODE").startsWith("URL")));
				if (System.getProperty("SESSION_TRACKING_MODE").startsWith("COOKIE")){
					...
					if (cookieType.contains("RUNTIME")){
						scc.setName("MyRuntimeCookieName");
		...
		try	{
	        ...
	        dynamicServletRegistration1.setInitParameter("WebClient1Results", "/WebClient3ResultsForm.jsp");
		}catch (Exception e){ 
	...
	scc.setName("MyStartupCookieName");
	scc.setAttribute("MyDynamicAttribute", "Dynamic Value");
    }
