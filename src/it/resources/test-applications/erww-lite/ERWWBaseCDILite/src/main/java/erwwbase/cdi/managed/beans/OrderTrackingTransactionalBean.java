...
import erwwbase.cdi.events.OrderTrackingEventTransactionalImpl;
...
@Priority(1)
@Dependent
public class OrderTrackingTransactionalBean implements Serializable  {
	
	private final static long serialVersionUID = 7529256653789513316L;
	
	@Inject @OrderTrackingTransactionalQualifier Event<OrderTrackingEvent> orderTrackingEvent;
	...
	private String className = (OrderTrackingTransactionalBean.class).getName();
	
	public OrderTrackingTransactionalBean(){
	}
		
	@ApplicationTraceInterceptor
	public void generateOrderTracking(@Observes @OrderTrackingTransactionalQualifier RequestOrderTrackingEvent requestOrderTrackingEvent) throws ConstraintViolationException, UserException {
		irwwbase.debugOut("<<< ThreadId: " + Thread.currentThread().getId() + ": In " + className + ": CDI Event 2: In OrderTrackingBean: Entered generateOrderTracking(@Observes @OrderTrackingTransactionalQualifier RequestOrderTrackingEvent requestOrderTrackingEvent: object identity = " + this);
		...
		try{
			orderTrackingEvent.fire(new OrderTrackingEventTransactionalImpl(orderTrackingPayload));
			irwwbase.debugOut("<<< ThreadId: " + Thread.currentThread().getId() + ": In " + className + ": CDI Event 3: orderTrackingEvent.fire(new OrderTrackingEventTransactionalImpl(orderTrackingPayload)): object identity = " + this);
		}catch (Exception e){			
			System.out.println("<<< ThreadId: " + Thread.currentThread().getId() + ": In " + className + ": Error in orderTrackingEvent.fire(new OrderTrackingEventTransactionalImpl(orderTrackingPayload)): " + e.getMessage());
			e.printStackTrace();
			throw new UserException("<<< ThreadId: " + Thread.currentThread().getId() + ": In " + className + ": Error in orderTrackingEvent.fire(new OrderTrackingEventTransactionalImpl(orderTrackingPayload)): ", e.getMessage());
		} 
	}
